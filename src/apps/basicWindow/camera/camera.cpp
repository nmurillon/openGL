...
uint loadTexture(const std::string &textureFile,
  ...
  const std::string texturePath{
      (libs::io::ProgramPath::getInstance().getProgramDir() /
       CAMERA_RESOURCES_FOLDER / textureFile)
          .string()};
...
int main(int argc, char **argv) {
  ...
  GLFWwindow *window =
      glfwCreateWindow(800, 600, "LearnOpenGL - Camera", NULL, NULL);
  if (window == NULL) {
 ...
 0.5f, -0.5f, -0.5f, 0.f, 0.f,// BOTTOM-RIGHT-DOWN
  ...
  view = glm::lookAt(glm::vec3(0.f, 0.f, 3.0f), glm::vec3(0.f, 0.f, 0.0f),
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    projection =
        glm::perspective(glm::radians(fov), 800.f / 600.f, 0.1f, 100.f);
